...
def strip_html(s):    
    return re.sub(HTML_TAG_PATTERN, '', html.unescape(s))
...
for (ln, col), p, d in generate_definitions():
    print('#:', bfn, ':', ln, sep='', file=OUTPUT)
    
    print("msgid", "_".join(p),  file=OUTPUT)
    print("msgstr", format(p[-1]),  file=OUTPUT)
    print(file=OUTPUT)
    
    print("msgid", "_".join(p + ("DOC",)),  file=OUTPUT)
